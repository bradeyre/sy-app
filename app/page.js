"use client";import{useEffect as _,useMemo as se,useRef as le,useState as m}from"react";import{formatZAR as B}from"@/lib/format";import{validateIdOrPassport as ft,digitsOnly as Me}from"@/lib/validation";import{isCourierCollectionDay as bt}from"@/lib/publicHolidays";import{cartBlocksCashPayout,isCashPayoutPreference}from"@/lib/luxuryWatchPaymentGate";import{cartBlocksHandbagCashPayout,isLuxuryHandbagCategory}from"@/lib/luxuryHandbagPaymentGate";import{HANDBAG_ESTIMATE_COPY,cartHasHandbagEstimate,cartHasHandbagEstimateUnavailable}from"@/lib/luxuryHandbagEstimate";import{HANDBAG_MISS_COPY,cartHasHandbagMiss}from"@/lib/luxuryHandbagMiss";import{leadRequiresBankDetails}from"@/lib/leadValidation";import HandbagMissStep from"./HandbagMissStep";function V(e){if(typeof window>"u")return e;const n=new URLSearchParams(window.location.search).get("site");if(!n)return e;const a=e.includes("?")?"&":"?";return`${e}${a}site=${encodeURIComponent(n)}`}const s={CATEGORY:"category",BRAND:"brand",MODEL:"model",QUOTE:"quote",FAULTS:"faults",ACCESSORIES:"accessories",EXTRAS:"extras",HANDBAG_MISS:"handbag_miss",PAYMENT:"payment",CALCULATING:"calculating",REVEAL:"reveal",DETAILS:"details",DONE:"done"},ie=[s.CATEGORY,s.MODEL,s.QUOTE,s.PAYMENT,s.DETAILS],ht={[s.CATEGORY]:"Device",[s.MODEL]:"Model",[s.QUOTE]:"Condition",[s.PAYMENT]:"Payment",[s.DETAILS]:"Your info"},$e={[s.BRAND]:s.MODEL,[s.HANDBAG_MISS]:s.MODEL,[s.FAULTS]:s.QUOTE,[s.ACCESSORIES]:s.QUOTE,[s.EXTRAS]:s.QUOTE,[s.CALCULATING]:s.PAYMENT,[s.REVEAL]:s.PAYMENT},gt={Sealed:"Brand new and unopened, still in its original packaging.",Mint:"Looks and works like new, no visible marks or faults.",Good:"Normal signs of everyday use, but fully working with no faults.",Poor:"Heavily used, or has a real fault, e.g. a cracked screen, a non-working button or port, or reduced battery health. Note: some faults may mean we're not able to buy back that particular item, you'll see if that applies on the next step."},je={Phone:<svg viewBox="0 0 24 24"fill="none"stroke="currentColor"strokeWidth="1.6"strokeLinecap="round"strokeLinejoin="round">
    ...
    </svg>,Vacuum:<svg viewBox="0 0 24 24"fill="none"stroke="currentColor"strokeWidth="1.6"strokeLinecap="round"strokeLinejoin="round">
      <path d="M9 3h4v7H9z"/><path d="M11 10v5"/><path d="M5 21h12a3 3 0 0 0-3-3H8a3 3 0 0 0-3 3z"/><circle cx="17.5" cy="6.5" r="2"/>
    </svg>,"Coffee Machine":<svg viewBox="0 0 24 24"fill="none"stroke="currentColor"strokeWidth="1.6"strokeLinecap="round"strokeLinejoin="round">
      <path d="M5 3h14v6H5z"/><path d="M9 9v3"/><path d="M7 21h10v-4H7z"/><path d="M12 12v5"/>
    </svg>,"Hair Care":<svg viewBox="0 0 24 24"fill="none"stroke="currentColor"strokeWidth="1.6"strokeLinecap="round"strokeLinejoin="round">
      <path d="M4 7h9a4 4 0 0 1 0 8H4z"/><path d="M8 15l1.5 6"/><path d="M17 11h4"/>
    </svg>,"Air Purifier":<svg viewBox="0 0 24 24"fill="none"stroke="currentColor"strokeWidth="1.6"strokeLinecap="round"strokeLinejoin="round">
      <rect x="7" y="3" width="10" height="18" rx="3"/><path d="M10 8h4"/><path d="M10 12h4"/><path d="M10 16h4"/>
    </svg>,"Action Camera":<svg viewBox="0 0 24 24"fill="none"stroke="currentColor"strokeWidth="1.6"strokeLinecap="round"strokeLinejoin="round">
      <rect x="3" y="6" width="18" height="13" rx="2"/><circle cx="12" cy="12.5" r="3.5"/><path d="M7 6V4h4v2"/>
    </svg>,Headphones:<svg viewBox="0 0 24 24"fill="none"stroke="currentColor"strokeWidth="1.6"strokeLinecap="round"strokeLinejoin="round">
      <path d="M4 14v-2a8 8 0 0 1 16 0v2"/><rect x="2.5" y="13.5" width="4" height="7" rx="1.6"/><rect x="17.5" y="13.5" width="4" height="7" rx="1.6"/>
    </svg>,Speaker:<svg viewBox="0 0 24 24"fill="none"stroke="currentColor"strokeWidth="1.6"strokeLinecap="round"strokeLinejoin="round">
    ...
    </svg>,"Luxury Watch":<svg viewBox="0 0 24 24"fill="none"stroke="currentColor"strokeWidth="1.6"strokeLinecap="round"strokeLinejoin="round">
...
function U(t,o){const mk=(T?.model||"").toLowerCase();return(i?.conditionFaults?.[t]||[]).filter(k=>k.decline||k.type==="battery_threshold"?o!=="Sealed":o==="Good"||o==="Poor").filter(k=>faultAppliesTo(k,mk)).map(k=>{const N=bnd&&k.byBrand&&k.byBrand[bnd];return N?{...k,...N}:k})}function q(t,o){return(i?.freeTextFaultCategories||[]).includes(t)&&(o==="Good"||o==="Poor")}function ot(t){if(isLuxuryHandbagCategory(g?.type)&&(t.estimateUnavailable||t.price==null)){setMissDraft({brand:T.brand||mb||bnd||"",model:T.label||T.model||"",colourOrMaterial:"",condition:t.condition,conditionLabel:t.conditionLabel||t.condition,serialNotes:"",handbagPhotos:[],entryId:t.entryId||null});setMissBack(s.QUOTE);n(s.HANDBAG_MISS);return}const d=(i?.aiDeductionCategories||[]).includes(g.type)&&t.condition==="Poor"?w.conditions.find(C=>C.condition==="Good"):null,k={key:`${T.model}-${w.capacity}-${t.condition}-${D.length}`,type:g.label,categoryType:g.type,model:T.label,capacity:w.capacity,condition:t.conditionLabel,conditionRaw:t.condition,basePrice:t.price==null?0:t.price,entryId:t.entryId,modelKey:T.model,brand:T.brand||mb||bnd||null,isEstimate:!!t.isEstimate||isLuxuryHandbagCategory(g.type),estimateUnavailable:!!t.estimateUnavailable||(isLuxuryHandbagCategory(g.type)&&t.price==null),goodCashBuy:w.conditions.find(C=>C.condition==="Good")?.price??null,goodPrice:d?.price??null,goodEntryId:d?.entryId??null};Te(k);const N=U(g.type,t.condition),y=q(g.type,t.condition);N.length>0||y||t.condition==="Mint"?(H([]),Z(""),X(""),n(s.FAULTS)):g.accessoryOptions?(G([]),n(s.ACCESSORIES)):g.extraAccessoryOptions?(we([]),ve(""),n(s.EXTRAS)):oe(k,[],{faultDeductionTotal:0,appliedFaults:[],pendingReviewFaults:[]})}function nt(t){Ae(t),g.accessoryOptions?(G([]),n(s.ACCESSORIES)):g.extraAccessoryOptions?(we([]),ve(""),n(s.EXTRAS)):oe(P,[],t)}function at(){const t=U(g?.type,P?.conditionRaw),o=t.filter(h=>h.type==="checkbox"),d=t.find(h=>h.type==="battery_threshold"),k=o.filter(h=>Se.includes(h.key)),N=[],y=[];let C=0;for(const h of k)h.deduction==null?y.push({key:h.key,label:h.label}):(N.push({key:h.key,label:h.label,deduction:h.deduction}),C+=h.deduction);if(d&&J!==""){const h=Number(J);if(Number.isFinite(h)&&h<d.threshold_pct){const E=`${d.label} (reported ${h}%)`;d.deduction==null?y.push({key:d.key,label:E}):(N.push({key:d.key,label:E,deduction:d.deduction}),C+=d.deduction)}}const S=Ee.trim().slice(0,500);q(g?.type,P?.conditionRaw)&&S&&y.push({key:"customer_described_issue",label:`Customer-described issue: "${S}"`}),nt({faultDeductionTotal:C,appliedFaults:N,pendingReviewFaults:y,hasReportedFault:N.length>0||y.length>0,faultDescriptionText:S})}function st(){const t=U(g?.type,P?.conditionRaw),o=q(g?.type,P?.conditionRaw);return t.length>0||o?s.FAULTS:s.QUOTE}function lt(t){fetch("/api/fault-price",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({category:t.categoryType,model:t.model,capacity:t.capacity,condition:t.condition,basePrice:t.basePrice,faults:t.pendingReviewFaults,appliedFaults:t.appliedFaults||[]})}).then(o=>o.ok?o.json():{proposals:[]}).then(o=>{const d=o.proposals||[];d.length!==0&&ee(k=>k.map(N=>{if(N.key!==t.key)return N;const y=[],C=[...N.aiProposedFaults||[]];let S=N.aiFaultDeductionTotal||0;for(const h of N.pendingReviewFaults||[]){const E=d.find(ae=>ae.key===h.key);E&&Number.isFinite(E.deduction)?(C.push({key:h.key,label:h.label,deduction:E.deduction,reasoning:E.reasoning||"",decline:!!E.decline}),S+=E.deduction):y.push(h)}const declined=N.declined||C.some(x=>x.decline);return{...N,pendingReviewFaults:y,aiProposedFaults:C,aiFaultDeductionTotal:S,declined}}))}).catch(()=>{})}function oe(t,o,d,F){const k=g?.accessoryOptions,N=k&&i?.accessoryBonus?.[k.settingsKey]||{},y=(k?.options||[]).filter(O=>o.includes(O.key)).map(O=>({key:O.key,label:O.label,bonus:Number(N[O.key])||0})),C=y.reduce((O,mt)=>O+mt.bonus,0),{faultDeductionTotal:S=0,appliedFaults:h=[],pendingReviewFaults:E=[],hasReportedFault:ae=!1,faultDescriptionText:dt=""}=d||{},Re=t.goodPrice!=null&&ae,ut=Re?t.goodPrice:t.basePrice,pt=Re?t.goodEntryId:t.entryId,{selectedExtras:eo=[],extrasText:to=""}=F||{},Be={...t,basePrice:ut,entryId:pt,accessories:y,accessoryBonusTotal:C,faultDeductionTotal:S,appliedFaults:h,pendingReviewFaults:E,faultDescription:dt,aiFaultDeductionTotal:0,aiProposedFaults:[],extras:eo.map(O=>({key:O.key,label:O.label,value:0,pending:!0})),extrasText:to,extrasTotalValue:0};ee(O=>[...O,Be]),(eo.length>0||to)&&yt_extraPrice(Be),E.length>0&&lt(Be),Te(null),G([]),H([]),Z(""),X(""),Ae({faultDeductionTotal:0,appliedFaults:[],pendingReviewFaults:[]}),we([]),ve(""),z(null),v(null),c(null),R(null),n(s.CATEGORY)}function yt_extraPrice(t){const F=g?.extraAccessoryOptions;if(!F)return;fetch("/api/extra-price",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({category:t.categoryType,model:t.model,capacity:t.capacity,extras:t.extras.map(o=>({key:o.key,label:o.label})),extrasText:t.extrasText||""})}).then(o=>o.ok?o.json():{estimates:[]}).then(o=>{const d=o.estimates||[];d.length!==0&&ee(k=>k.map(N=>{if(N.key!==t.key)return N;const y=N.extras.map(S=>{const h=d.find(E=>E.key===S.key);return h?{...S,value:h.value,reasoning:h.reasoning||"",pending:!1}:S}),C=d.filter(S=>S.key.startsWith("freetext_item_")).map(S=>({key:S.key,label:S.label,value:S.value,reasoning:S.reasoning||"",pending:!1}));return{...N,extras:[...y,...C],extrasTotalValue:[...y,...C].reduce((S,h)=>S+h.value,0)}}))}).catch(()=>{})}function it(t){ee(o=>o.filter(d=>d.key!==t))}function openHandbagMiss(prefill){setMissDraft({brand:prefill?.brand||mb||"",model:prefill?.model||"",colourOrMaterial:"",condition:prefill?.condition||"",conditionLabel:prefill?.conditionLabel||"",serialNotes:"",handbagPhotos:[],entryId:prefill?.entryId||null});setMissBack(prefill?.back||(mb?s.MODEL:s.BRAND));n(s.HANDBAG_MISS)}function completeHandbagMiss(){const brand=missDraft.brand.trim(),model=missDraft.model.trim(),Be={key:`handbag-miss-${brand}-${model}-${D.length}`,type:g?.label||"Luxury Handbag",categoryType:g?.type||"Luxury Handbag",model,modelKey:model.toLowerCase(),brand,capacity:"N/A",condition:missDraft.conditionLabel||missDraft.condition,conditionRaw:missDraft.condition,basePrice:0,entryId:missDraft.entryId||null,isEstimate:!0,estimateUnavailable:!0,missFlow:!0,colourOrMaterial:missDraft.colourOrMaterial.trim(),serialNotes:(missDraft.serialNotes||"").trim(),handbagPhotos:missDraft.handbagPhotos||[],accessories:[],accessoryBonusTotal:0,extras:[],extrasTotalValue:0,appliedFaults:[],pendingReviewFaults:[],aiProposedFaults:[],faultDeductionTotal:0,aiFaultDeductionTotal:0,goodCashBuy:null};ee(O=>[...O,Be]);setMissDraft({brand:"",model:"",colourOrMaterial:"",condition:"",conditionLabel:"",serialNotes:"",handbagPhotos:[],entryId:null});z(null);v(null);c(null);R(null);n(s.CATEGORY)}function Ie(t,o){const d=t==="selfie"?He:Qe,k=t==="selfie"?"selfiePath":"idDocumentPath";if(!o)return;d({status:"uploading",fileName:o.name,error:null});const N=(o.name.split(".").pop()||"jpg").toLowerCase();fetch("/api/upload-url",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({fileExt:N})}).then(y=>y.json()).then(({uploadUrl:y,path:C,error:S})=>{if(S)throw new Error(S);return fetch(y,{method:"PUT",headers:{"Content-Type":o.type||"application/octet-stream"},body:o}).then(h=>{if(!h.ok)throw new Error("Upload failed");return C})}).then(y=>{Fe(C=>({...C,[k]:y})),d({status:"done",fileName:o.name,error:null})}).catch(y=>d({status:"error",fileName:o.name,error:y.message}))}const rt=D.reduce((t,o)=>t+Math.max(0,o.basePrice+o.accessoryBonusTotal+(o.extrasTotalValue||0)-(o.faultDeductionTotal||0)-(o.aiFaultDeductionTotal||0)),0),yg=se(()=>cartBlocksCashPayout(D)||cartBlocksHandbagCashPayout(D),[D]),handbagGated=se(()=>cartBlocksHandbagCashPayout(D),[D]),ne=se(()=>{if(!i)return[];const consignHelper=handbagGated?HANDBAG_ESTIMATE_COPY.consignHelper:`If you don't mind waiting for your payment, consignment is for you. Average sale period is around 13 days. You'll get +${i.consignmentPct}% extra until 30 September, then it returns to 10%.`;const t=[{key:"consignment",pct:i.consignmentPct,shortLabel:"Epic Deals Consignment",label:`+${i.consignmentPct}% consignment until 30 Sep`,helper:consignHelper},{key:"voucher",pct:i.voucherPct,shortLabel:"Epic Deals Voucher",label:`Epic Deals Voucher (${i.voucherPct}% Extra!)`,helper:"The biggest bonus we offer, redeemable in our online store on whatever you like."},{key:"eft",pct:0,shortLabel:"EFT",label:"EFT",helper:"Paid as soon as we've tested your device, no bonus, straight into your bank account."}];return yg?t.filter(o=>o.key!=="eft"):t},[i,yg,handbagGated]),pay=yg&&isCashPayoutPreference(M)?"consignment":M,_e=ne.find(t=>t.key===pay)?.pct||0,W=Math.round((rt*(1+_e/100)+(cpn?.bonus||0))*100)/100;_(()=>{setCpn(null),setCpnErr(null)},[rt,D.length]);function applyCoupon(){const t=cpnIn.trim();if(t){setCpnBusy(!0),setCpnErr(null),fetch(V("/api/coupon"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({code:t,subtotal:rt,itemCount:D.length})}).then(o=>o.json()).then(o=>{o.ok?(setCpn({code:o.code,bonus:o.bonus,description:o.description}),setCpnErr(null)):(setCpn(null),setCpnErr(o.error||"That coupon code isn't valid"))}).catch(()=>{setCpn(null),setCpnErr("Could not check that code right now")}).finally(()=>setCpnBusy(!1))}}function ct(t){if(t.preventDefault(),D.some(o=>o.declined))return void L("One of your items can't be bought in its current condition, please remove it to continue.");Pe(!0),L(null),fetch(V("/api/lead"),{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({items:D.map(o=>({type:o.type,categoryType:o.categoryType,model:o.model,modelKey:o.modelKey,capacity:o.capacity,condition:o.conditionRaw,price:o.basePrice,entryId:o.entryId,accessories:o.accessories,accessoryBonusTotal:o.accessoryBonusTotal,faultDeductionTotal:o.faultDeductionTotal||0,appliedFaults:o.appliedFaults||[],pendingReviewFaults:o.pendingReviewFaults||[],aiFaultDeductionTotal:o.aiFaultDeductionTotal||0,aiProposedFaults:o.aiProposedFaults||[],faultDescription:o.faultDescription||"",extras:o.extras||[],extrasText:o.extrasText||"",extrasTotalValue:o.extrasTotalValue||0,isEstimate:!!o.isEstimate,estimateUnavailable:!!o.estimateUnavailable,missFlow:!!o.missFlow,brand:o.brand||null,colourOrMaterial:o.colourOrMaterial||"",serialNotes:o.serialNotes||"",handbagPhotos:o.handbagPhotos||[]})),quotedTotal:W,paymentPreference:pay,paymentBonusPct:_e,couponCode:cpn?.code||null,authenticityAccepted:!!Oe.authenticityAccepted,...Oe,renderedAt:Le.current})}).then(o=>o.json()).then(o=>{if(o.error)throw new Error(o.error);const k=o.reference||o.id;Xe(k),n(s.DONE);window.parent?.postMessage({type:"epic-calc-lead",reference:k,site:new URLSearchParams(window.location.search).get("site")||undefined,total:W},"*")}).catch(o=>L(o.message)).finally(()=>Pe(!1))}return<div ref={te}className="mx-auto w-full max-w-xl px-4 py-6 font-sans">
      ...
      <div key={e}className="step-enter">
        ...
        {e===s.BRAND&&<BrandStep category={g}brands={[...new Set((r||[]).map(t=>t.brand).filter(Boolean))].sort((t,o)=>t.localeCompare(o))}onChoose={t=>{setMb(t),x(""),n(s.MODEL)}}onBack={()=>n(s.CATEGORY)}onMiss={isLuxuryHandbagCategory(g?.type)?()=>openHandbagMiss({brand:"",back:s.BRAND}):null}/>}

        {e===s.MODEL&&<At category={g}brand={mb}models={mb?(r||[]).filter(t=>t.brand===mb):r}loading={We}search={A}onSearch={x}onChoose={tt}onBack={()=>n([...new Set((r||[]).map(t=>t.brand).filter(Boolean))].length>1?s.BRAND:s.CATEGORY)}onMiss={isLuxuryHandbagCategory(g?.type)?()=>openHandbagMiss({brand:mb||"",back:s.MODEL}):null}/>}

        {e===s.HANDBAG_MISS&&<HandbagMissStep draft={missDraft}onChange={setMissDraft}onBack={()=>n(missBack)}onContinue={completeHandbagMiss}/>}

        {e===s.QUOTE&&<Dt model={T}capacities={j}notice={qNote}activeCapacity={w}onChooseCapacity={R}onChooseCondition={ot}onBack={()=>n(s.MODEL)}categoryType={g?.type}/>}
        ...
        {e===s.PAYMENT&&<Pt options={ne}selected={pay}onSelect={qe}onContinue={()=>n(s.CALCULATING)}onBack={()=>n(s.CATEGORY)}cashGated={yg} handbagGated={handbagGated}/>}

        {e===s.CALCULATING&&<Lt onDone={()=>n(s.REVEAL)}/>}

        {e===s.REVEAL&&<It total={W}items={D}paymentLabel={ne.find(t=>t.key===pay)?.shortLabel}handbagEstimate={cartHasHandbagEstimate(D)}handbagEstimateUnavailable={cartHasHandbagEstimateUnavailable(D)&&!D.some(t=>!t.estimateUnavailable&&(t.basePrice||0)>0)}coupon={cpn}couponInput={cpnIn}onCouponInput={setCpnIn}couponError={cpnErr}couponBusy={cpnBusy}onApplyCoupon={applyCoupon}onRemoveCoupon={()=>{setCpn(null),setCpnErr(null),setCpnIn("")}}onContinue={()=>n(s.DETAILS)}onBack={()=>n(s.PAYMENT)}/>}

        {e===s.DETAILS&&<Mt form={Oe}setForm={Fe}onSubmit={ct}submitting={Je}onBack={()=>n(s.REVEAL)}total={W}paymentPreference={pay}idUpload={Ve}onIdFile={t=>Ie("id",t)}selfieUpload={Ke}onSelfieFile={t=>Ie("selfie",t)}handbagMiss={cartHasHandbagMiss(D)} requireBank={leadRequiresBankDetails({paymentPreference:pay,items:D})}/>}

        {e===s.DONE&&<$t leadId={Ze}total={W}items={D}handbagMiss={cartHasHandbagMiss(D)}/>}
      </div>
        ...
        {e.map(a=><li key={a.key}className="item-in flex items-center gap-3 border-t border-line px-4 py-3">
            ...
            <span className="min-w-0 flex-1">
              <span className="block truncate text-base font-medium text-fg">
                {a.brand?`${a.brand} `:""}{a.model} {a.capacity!=="N/A"?`(${a.capacity})`:""}
              </span>
              <span className="block text-sm text-muted">
                {a.missFlow||a.estimateUnavailable?`${a.condition} · ${HANDBAG_ESTIMATE_COPY.unavailableLabel}`:a.condition}
    ...
    </div>}function BrandStep({category:e,brands:n,onChoose:a,onBack:l,onMiss:om}){return<div className="space-y-2">
      <I onClick={l}label="Back"/>
      <p className="mb-2 text-base font-medium text-muted">Who makes your {e.label}?</p>
      {n.map(f=><Ne key={f}onClick={()=>a(f)}>
          {f}
        </Ne>)}
      {om&&<button type="button"onClick={om}className="mt-3 w-full rounded-xl border border-brand bg-brand/5 px-4 py-3.5 text-left text-base font-medium text-brand">
          {HANDBAG_MISS_COPY.missEntryCta}
        </button>}
    </div>}function At({category:e,brand:u2,models:n,loading:a,search:l,onSearch:f,onChoose:p,onBack:i,onMiss:om}){/* With the brand already chosen and named in the question above, repeating it on every row is noise: "Which Sage coffee machine do you have?" followed by "Sage Bambino Plus". Stripped for display only -- the value sent on is untouched, because Airtable matches on the full name. */const trim=t=>u2&&t.toLowerCase().startsWith(u2.toLowerCase()+" ")?t.slice(u2.length+1):t;const u=n?.filter(r=>r.label.toLowerCase().includes(l.trim().toLowerCase()));return<div className="space-y-2">
    ...
    </div>}function Dt({model:e,capacities:n,notice:no,activeCapacity:a,onChooseCapacity:l,onChooseCondition:f,onBack:p,categoryType:ct}){const i=!a&&n?.length>1;const hb=isLuxuryHandbagCategory(ct);return<div className="space-y-2">
      <I onClick={p}label="Back"/>
      {hb&&<div className="rounded-xl border border-brand/30 bg-brand/5 p-4 mb-1">
          <p className="text-base font-semibold text-fg mb-1">{HANDBAG_ESTIMATE_COPY.label}</p>
          <p className="text-base text-muted leading-relaxed">{HANDBAG_ESTIMATE_COPY.quoteDisclaimer}</p>
          <p className="text-sm text-muted leading-relaxed mt-2">{HANDBAG_ESTIMATE_COPY.helperUnderNumber}</p>
        </div>}
      ...
      {a&&<>
          ...
          {a.conditions.map(u=><St key={u.condition}onClick={()=>f(u)}dot={xt[u.condition]||"#9ca3af"}label={u.estimateUnavailable?`${u.conditionLabel} · ${HANDBAG_ESTIMATE_COPY.unavailableLabel}`:u.isEstimate?`${u.conditionLabel} · ${HANDBAG_ESTIMATE_COPY.label}`:u.conditionLabel}explainer={hb?HANDBAG_ESTIMATE_COPY.quoteDisclaimer:gt[u.condition]}/>)}
        </>}
...
Je=Qe==="Mint",[Ze,Xe]=m(!1);return A&&z?<div className="space-y-2">
    ...
    </button>}function Pt({options:e,selected:n,onSelect:a,onContinue:l,onBack:f,cashGated:yg,handbagGated:hg}){if(e.length===0)return<Ce rows={3}/>;const p=e.find(i=>i.key===n)||e[0];return<div className="space-y-2">
      <I onClick={f}label="Back"/>
      <p className="mb-2 text-base font-medium text-muted">How would you like to be paid?</p>
      {yg&&<p className="mb-2 text-sm text-muted">{hg?"Instant EFT is not available for luxury handbags. You can choose consignment or an Epic Deals voucher.":"Instant EFT is not available for this watch. You can choose consignment or an Epic Deals voucher."}</p>}
 ...
 */
function It({total:e,items:n,paymentLabel:a,onContinue:l,onBack:f,coupon:S,couponInput:E,onCouponInput:C,couponError:N,couponBusy:k,onApplyCoupon:y,onRemoveCoupon:h,handbagEstimate:hbEst,handbagEstimateUnavailable:hbUnavail}){
  ...
  <div className="space-y-4">
    ...
    <div className="rounded-2xl border border-line bg-card px-5 py-10 text-center">
      <p className="reveal-rise text-xs font-medium uppercase tracking-[0.18em] text-muted">
        {hbEst||hbUnavail?HANDBAG_ESTIMATE_COPY.label:"Your offer"}
      </p>
      {(hbEst||hbUnavail)&&<p className="reveal-rise mt-3 text-sm font-medium text-muted">{HANDBAG_ESTIMATE_COPY.primary}</p>}
      ...
      <p className="reveal-rise reveal-delay-1 tabular mt-8 text-[clamp(2.75rem,13vw,4.5rem)] font-bold leading-[0.95] tracking-[-0.03em] text-fg">
        {hbUnavail?HANDBAG_ESTIMATE_COPY.unavailableLabel:B(p)}
      </p>
      {hbEst&&!hbUnavail&&<p className="reveal-rise reveal-delay-2 mx-auto mt-4 max-w-sm text-sm leading-relaxed text-muted">{HANDBAG_ESTIMATE_COPY.helperUnderNumber}</p>}
      {hbUnavail&&<p className="reveal-rise reveal-delay-2 mx-auto mt-4 max-w-sm text-sm leading-relaxed text-muted">{HANDBAG_ESTIMATE_COPY.unavailableHelper}</p>}
      ...
      <p className="reveal-rise reveal-delay-3 mt-7 text-base text-muted">
        {hbEst||hbUnavail?`Paid via ${a}.`:He?`Paid via ${a}, that's your total, all in.`:`Paid via ${a}, that's your total, all in, no surprises.`}
      </p>
      ...
      {He&&!hbEst&&!hbUnavail&&<p className="reveal-rise reveal-delay-3 mx-auto mt-4 max-w-sm rounded-lg border border-line bg-canvas px-4 py-3.5 text-left text-base leading-relaxed text-muted">
          {Ge>0
    ...
    <button type="button"onClick={l}className="w-full rounded-xl bg-brand px-4 py-3.5 font-semibold text-white transition hover:bg-brand-600">
      ...
      <F label="Email"type="email"required={hm}value={e.email}onChange={x("email")}/>
      ...
      <div className="rounded-lg border border-line bg-canvas p-3">
        ...
        {hm&&<label className="mt-3 flex items-start gap-2 text-base text-fg">
          <input type="checkbox"required checked={e.authenticityAccepted}onChange={c=>n(w=>({...w,authenticityAccepted:c.target.checked}))}className="mt-0.5 h-4 w-4 rounded border-line text-brand focus:ring-brand"/>
          <span>{HANDBAG_MISS_COPY.counterfeitDisclaimer}</span>
        </label>}
        {hm&&<p className="mt-3 text-sm text-muted">
          <a href={HANDBAG_MISS_COPY.authHelperUrl}target="_blank"rel="noreferrer"className="font-medium text-brand hover:underline">{HANDBAG_MISS_COPY.authHelperCta}</a>
          {" "}{HANDBAG_MISS_COPY.authHelperHint}
        </p>}
      </div>
    ...
    </form>}const Ye=["We'll take it from here.","Kettle's on, we won't be long.","No further haggling required, promise.","Sit back, we've got this one."];function $t({leadId:e,total:n,items:a,handbagMiss:hm}){const[l]=m(()=>Ye[Math.floor(Math.random()*Ye.length)]);return<div className="rounded-xl border border-green-200 bg-green-50 p-5 text-center">
      <p className="text-lg font-semibold text-positive">{hm?HANDBAG_MISS_COPY.thankYou:"Thanks, we've got your details!"}</p>
      {hm?<p className="mt-2 text-base text-positive">{HANDBAG_MISS_COPY.promise48h}</p>:<p className="mt-2 text-base text-positive">
      ...
      {hm&&<p className="mt-3 text-left text-sm leading-relaxed text-positive">{HANDBAG_MISS_COPY.counterfeitDisclaimer}</p>}
      {!hm&&<p className="mt-2 text-sm text-positive">{l}</p>}
