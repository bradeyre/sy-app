...
test("pins Luxury Handbag under Luxury Watch when both are present", () => {
  const input = [
    { type: "Phone" },
    { type: "Watch" },
    { type: "Console" },
...
test("leaves Luxury Handbag in place when Watch family is absent", () => {
  const input = [{ type: "Phone" }, { type: "Luxury Handbag" }, { type: "Console" }];
  assert.equal(orderWatchFamily(input), input);
});

test("already-pinned Watch / Luxury Watch / Luxury Handbag is untouched", () => {
  const input = [
    { type: "Phone" },
    { type: "Watch" },
    { type: "Luxury Watch" },
