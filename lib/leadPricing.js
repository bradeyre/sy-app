...
export async function revalidateLeadPricing({ site, items, paymentPreference, quoteRef }) {
  ...
  for (const item of items) {
    ...
    if (row && row.condition === item.condition) {
      ...
      if (fixedTotal + aiTotal > 0) {
        ...
        const { rows: poorRows } = await query(
          `select buy_price from calc.buy_prices_public bp
    ...
    if (itemDeclined) {
      ...
      flag(`item_declined:${item.modelKey}:${declineReasons.join(",")}`, true);
      itemContribution = 0;
    }
  ...
  //
  // A dropped DECLINED proposal is worse than a dropped ordinary one -- it's
  // exactly the shape a customer would use to route around a "we can't buy
  // this" verdict (untick the fault that triggered it, keep everything else),
  // so it's flagged for review the same as any other omission would be; there
  // is no separate item left in this lead to zero out once it's untraceable.
  if (quoteRef) {
    for (const p of proposalRes.rows) {
      if (usedProposalIds.has(p.id)) continue;
      if (p.declined) {
        flag(`declined_fault_omitted:${p.category}:${p.fault_key}`, true);
        continue;
      }
