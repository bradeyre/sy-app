...
export async function revalidateLeadPricing({ site, items, paymentPreference, quoteRef }) {
  ...
  for (const item of items) {
    ...
    // Luxury Handbag: never treat thin cash buy_prices as a firm offer.
    // Recompute displayed estimate from Luxity sell-sample median × 0.9.
    // Missing sample → R0 + review (estimate on inspection), not invent.
    // Miss-flow (no Luxity match / manual bag) never invents a Rand amount.
    if (item.missFlow) {
      basePrice = 0;
      flag(`handbag_miss_quote:${item.modelKey || item.model}`, true);
    } else if (isLuxuryHandbagCategory(item.categoryType || row?.type)) {
      const { rows: sampleRows } = await query(
        `select luxity_sample_median
          ...
          limit 1`,
        [item.modelKey || item.model, catalogBrand || item.brand || null]
      );
      const median = sampleRows[0]?.luxity_sample_median;
      const estimate = displayedHandbagEstimate(median);
      if (estimate == null) {
        basePrice = 0;
        flag(`handbag_estimate_unavailable:${item.modelKey}`, true);
      } else {
        basePrice = estimate;
        flag(`handbag_estimate:${item.modelKey}`);
      }
    }
