...
import { orderWatchFamily } from "@/lib/categoryOrder";
...
function shape(row) {
  // category_filter order is the cockpit default. Watch, Luxury Watch and
  // Luxury Handbag are pinned as a family (Watch first) so a later luxury
  // row cannot land at the bottom of the sell-form list.
  const categories = orderWatchFamily(
    (row.category_filter || []).map((type) => ({
      type,
  ...
  return {
    ...
    /**
     * Every catalogue query does `brand ilike any($1)`, and an empty array
     * matches nothing, so a tenant with no brand restriction would have shown
     * an empty shop. Epic Deals is the first such tenant: it sells the whole
     * catalogue rather than one maker's slice.
     *
     * `['%']` says "any brand" in the language the queries already speak, so
     * none of the six call sites need to know about it, and a brand added to
     * the catalogue tomorrow appears without a config change.
     */
    where: { brand: row.brand_filter?.length ? row.brand_filter : ["%"] },
