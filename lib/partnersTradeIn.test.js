...
import { validatePartnerLead, buildJsonLd, FAQ, SEO, CANONICAL_URL, HERO } from "./partnersTradeIn.js";
...
test("JSON-LD carries Organization, WebPage and every FAQ", () => {
  ...
  assert.equal(graph[1].url, "https://epicdeals.co.za/partners/trade-in-as-payment/");
  assert.equal(CANONICAL_URL, graph[1].url);
  assert.equal(HERO.h1, "Pay with the stuff they already own.");
});
