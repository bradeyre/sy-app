 ...
 *
 * Unknown brands are mapped to Other before the write. If a named-field 422
 * still happens (Cannot parse, INVALID_VALUE_FOR_COLUMN,
 * INVALID_MULTIPLE_CHOICE_OPTIONS, UNKNOWN_FIELD_NAME), retry once with that
 * field stripped -- Device Brand is rewritten as Other first.
 */
...
n&&(o["Client ID Doc"]=n);
r&&(o["Client Photo"]=r);
const liveChoices=await loadDeviceBrandChoices();const c=(await Promise.all(i.map(s=>M({...s,airtableSource:e.airtableSource,paymentPreference:e.paymentPreference,paymentBonusPct:e.paymentBonusPct},t,liveChoices)))).map(s=>({fields:{...o,...s}}));
const recordIds=[];
