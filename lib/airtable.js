 ...
 *
 * Lead POSTs send typecast so a new calculator brand becomes a select option.
 * If a named-field 422 still happens (Cannot parse, INVALID_VALUE_FOR_COLUMN,
 * INVALID_MULTIPLE_CHOICE_OPTIONS, UNKNOWN_FIELD_NAME), retry once with that
 * field stripped -- Device Brand is rewritten as Other first. A record with
 * one blank cell is recoverable; a lead that never arrives is not.
 */
async function postLeadRecords(records){
try{return await f(`${u}/${g}`,{method:"POST",body:JSON.stringify(airtableLeadWriteBody(records))})}
catch(err){
...
return f(`${u}/${g}`,{method:"POST",body:JSON.stringify(airtableLeadWriteBody(rewritten))});
}
}
