import{createSignedReadUrl as P}from"@/lib/storage";import{formatZAR as Z}from"@/lib/format";import{airtableStripRetryLog,itemOfferAmount,itemQuotedValue,notesWithQuoteRef,resolvePaymentPreference,rewriteRecordsForAirtableRetry}from"@/lib/airtableLeadSync";const h=process.env.AIRTABLE_API_KEY,u="appMB4HF3PkGe2rZd",g="tblx9AkbkYzo8Cqhu",p="tblTYFnQ9cn7SNKXi",I="https://api.airtable.com/v0";async function f(e,i={}){const t=await fetch(`${I}/${e}`,{...i,headers:{Authorization:`Bearer ${h}`,"Content-Type":"application/json",...i.headers}});if(!t.ok){const n=await t.text().catch(()=>"");throw new Error(`Airtable ${i.method||"GET"} ${e} failed: ${t.status} ${n}`)}return t.json()}function l(e){return String(e||"").trim().replace(/\s+/g," ").toLowerCase()}function d(e,i){if(!i)return null;const t=l(i);return e.find(r=>l(r)===t)||null}const DEVICE_BRAND_CHOICES=["Apple","Samsung","Sony","Microsoft","Nintendo","Steam","Lenovo","Asus","Acer","Dell","HP","MSI","Razer","Google","Huawei","Honor","Oppo","Vivo","Xiaomi","Redmi","OnePlus","Nothing","CMF by Nothing","LG","HTC","Meta","Garmin","GoPro","Beats","Bose","Sonos","Canon","Logitech","Hisense","Philips","Siemens","Nespresso","WD","Bowers & Wilkins","DJI","DeLonghi","Dyson","Ecovacs","Gaggia","Jura","Miele","Philips Saeco","Roborock","Sage","Shark","ghd","Other"];
/**
 ...
 *
 * When Airtable names the column it could not parse (or rejects a select
 * option), drop that one field and write the lead again. A record with one
 * blank cell is recoverable; a lead that never arrives is not. Retried once
 * only. Payment Preference / Quoted Value / Offer Amount / Notes (quote
 * ref) are never dropped without a structured error log naming the field,
 * lead, and ref.
 */
async function postLeadRecords(records, context = {}) {
  try {
    return await f(`${u}/${g}`, { method: "POST", body: JSON.stringify({ records }) });
  } catch (err) {
    const rewritten = rewriteRecordsForAirtableRetry(records, err);
    if (!rewritten) {
      console.error(JSON.stringify({
        event: "airtable_sync_write_failed",
    ...
    console.error(JSON.stringify(airtableStripRetryLog({
      leadId: context.leadId,
      quoteRef: context.quoteRef,
      strippedFields: rewritten.strippedFields,
      strippedCritical: rewritten.strippedCritical,
      err,
    })));
    return f(`${u}/${g}`, { method: "POST", body: JSON.stringify({ records: rewritten.records }) });
  }
}
...
export async function syncLeadToAirtable({ lead: e, items: i, brand: t, leadId: leadId } = {}) {
  ...
  if (!h) {
    ...
    console.error(JSON.stringify({ event: "airtable_sync_skipped", leadId: leadId || null, quoteRef, error }));
    return;
  }
  try {
    ...
    n && (o["Client ID Doc"] = n);
    r && (o["Client Photo"] = r);
    if (e.paymentPreference && !resolvePaymentPreference(e.paymentPreference, e.paymentBonusPct)) {
      console.error(JSON.stringify({
        event: "airtable_payment_preference_unmapped",
        leadId: leadId || null,
        quoteRef,
        paymentPreference: e.paymentPreference,
        paymentBonusPct: e.paymentBonusPct,
      }));
    }
    const c = (await Promise.all(i.map((s) => M({
      ...s,
      airtableSource: e.airtableSource,
      paymentPreference: e.paymentPreference,
      paymentBonusPct: e.paymentBonusPct,
    }, t)))).map((s) => ({ fields: { ...o, ...s } }));
    for (const s of x(c, 10)) await postLeadRecords(s, { leadId, quoteRef });
    console.log(JSON.stringify({ event: "airtable_sync_ok", leadId: leadId || null, quoteRef, items: i?.length || 0 }));
  } catch (err) {
    console.error(JSON.stringify({
