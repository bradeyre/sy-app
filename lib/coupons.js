...
export async function evaluateCoupon({ code, siteKey, subtotal, itemCount = 1 }) {
  ...
  const { rows } = await query(
    `select id, code, description, kind, value, max_bonus, min_quote_total,
            sites, max_uses, used_count, valid_from, valid_until, active, per_item
     from calc.coupons
     where upper(code) = $1
     limit 1`,
    [normalised]
  ...
  // A per-item fixed coupon pays for each device in the submission, so two
  // iPhones in one basket earn twice. Percentage coupons already grow with the
  // basket, so multiplying those would pay the bonus twice over.
  const devices = Math.max(1, Math.floor(Number(itemCount) || 1));
  let bonus =
    coupon.kind === "fixed"
      ? Number(coupon.value) * (coupon.per_item ? devices : 1)
