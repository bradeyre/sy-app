...
/**
 * True when every cart line is still waiting on a quote: Luxury Handbag
 * miss, or any missFlow / estimateUnavailable line. Mixed carts with a
 * priced item are false so the normal confirmation path still runs.
 */
export function cartIsQuotePendingMiss(items) {
  const list = items || [];
  if (list.length === 0) return false;
  return list.every(
    (item) =>
      isHandbagMissItem(item) || Boolean(item?.missFlow) || Boolean(item?.estimateUnavailable)
  );
}

/**
 * Post-submit jobs for POST /api/lead.
 *
 * notifyHandbagMiss — Resend ops (jarred@) + 48h miss customer email.
 * syncLeadToAirtable — SHGA create; n8n sends the payday/courier customer
 *   ack from that write. Skip it for quote-pending miss carts so they do
 *   not also get "Relax… payday's on its way" / packing-photos.
 */
export function leadPostSubmitJobs(items) {
  return {
    notifyHandbagMiss: cartHasHandbagMiss(items),
    syncLeadToAirtable: !cartIsQuotePendingMiss(items),
  };
}
